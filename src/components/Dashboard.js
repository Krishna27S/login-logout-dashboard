...
import { Menu, X, Home, User, Settings, LogOut, Bell, Search, ChevronDown } from 'lucide-react';
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <div
      ...
      } w-64 bg-indigo-800 text-white transition duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0`}
    >
      <div className="flex items-center justify-center h-20 bg-indigo-900">
      ...
      <nav className="mt-10">
        {navItems.map((item, index) => (
          <a
            key={index}
            href={item.href}
            className="flex items-center px-6 py-3 text-gray-100 hover:bg-indigo-700 transition duration-200 ease-in-out"
          >
      ...
      <div className="flex-1 flex justify-center px-2 lg:ml-6 lg:justify-end">
        <div className="max-w-lg w-full lg:max-w-xs">
          ...
          <div className="relative">
            ...
            <input id="search" className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 sm:text-sm" placeholder="Search" type="search" />
          </div>
        </div>
      </div>
        ...
        <div className="ml-3 relative">
          <div>
            <button className="max-w-xs bg-white flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500" id="user-menu" aria-haspopup="true">
              ...
              <span className="ml-3 text-gray-700 text-sm font-medium lg:block"><span className="sr-only">Open user menu for </span>Krishna Shekhar</span>
              <ChevronDown className="flex-shrink-0 ml-1 h-5 w-5 text-gray-400" aria-hidden="true" />
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <main className="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100">
          <div className="container mx-auto px-6 py-8">
            <h3 className="text-gray-700 text-3xl font-semibold">Dashboard</h3>
            <div className="mt-4">
